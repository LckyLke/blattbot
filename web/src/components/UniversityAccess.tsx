...
export default function UniversityAccess() {
  ...
  const button = "rounded-lg border border-rule px-3 py-1.5 text-xs text-paper disabled:opacity-50";
  return <div className="max-w-[560px] space-y-4">
    ...
    {login ? <div className="space-y-3 rounded-lg border border-leaf/40 bg-ink p-3">
    ...
    </div> : <form className="space-y-3 rounded-lg border border-rule bg-ink p-3" onSubmit={event => { event.preventDefault(); void action(() => api.startPublisherLogin(url, institution)); }}>
      <label className="block text-xs text-graphite">Publisher
        <select aria-label="Publisher" className="mt-1 block w-full rounded-lg border border-rule bg-ink px-2 py-2 text-paper" value={publishers.some(([, value]) => value === url) ? url : "custom"} onChange={event => setUrl(event.target.value === "custom" ? "" : event.target.value)}>
      ...
      <label className="block text-xs text-graphite">Publisher website
        <input required type="url" value={url} placeholder="https://publisher.example" onChange={event => setUrl(event.target.value)} className="mt-1 block w-full rounded-lg border border-rule bg-ink px-2 py-2 text-paper" />
      </label>
      <label className="block text-xs text-graphite">Institution label (optional)
        <input value={institution} maxLength={200} placeholder="Your university or library" onChange={event => setInstitution(event.target.value)} className="mt-1 block w-full rounded-lg border border-rule bg-ink px-2 py-2 text-paper" />
      </label>
    ...
    {connections.length > 0 && <ul className="space-y-3" aria-label="Saved publisher connections">
      {connections.map(connection => <li key={connection.id} className="rounded-lg border border-rule p-3">
